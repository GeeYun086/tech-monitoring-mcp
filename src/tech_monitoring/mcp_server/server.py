...
impact_signals.relevance_rerank_score는 "AX 시장과 관련 있는가"를 재확인한
참고 신호일 뿐, 기사를 걸러내는 기준으로 이미 쓰였다는 뜻이 아니다(모든
기사가 여전히 반환 대상이다). 값 자체는 0.0000~0.01대로 극히 작게
압축돼 있어 "0.005점짜리라 관련 없다"처럼 절대값으로 단정하지 말 것 —
같은 응답 안에서 다른 기사와 상대적으로 비교하는 용도로만 쓸 것.
""".strip()
