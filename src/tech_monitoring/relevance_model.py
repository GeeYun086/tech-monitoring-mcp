...
def build_text(row: dict) -> str:
    ...
    **시장 구분 없이 제목+요약만 본다(2026-08-24 담당자 결정)** — 예전엔
    "기사 × 시장" 쌍의 속성이라며 고정 키워드를 앞에 붙였는데(004 마이그레이션
    헤더 (3)), 수집 자체가 애초에 시장과 무관한 공용 풀이라 시장을 3개로
    나눠보는 게 의미 없다는 판단 아래 "이 기사가 일반적으로 도움되는가" 하나로
    합쳤다. 그 전에 3개 시장에 걸쳐 쌓인 라벨은 버리지 않고 그대로 하나의
    학습 데이터로 합쳐 쓴다 — row에 fixed_keyword가 남아있어도 여기선 안 본다
    (per_market_metrics 등 진단용 분해에는 여전히 남아 있어 참고용으로 쓸 수
    있다).
    """
    parts = [row["title"] or ""]
    if row.get("snippet"):
