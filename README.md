...
## 관련도 분류기 — 학습 원리와 성능 측정

기사가 "이 팀에 도움되는가"는 LLM이 아니라, 팀이 직접 누른 👍/👎로 학습한
로컬 분류기가 판단합니다. API 호출이 없어 요금제·장애와 무관하게 항상
동작합니다.

### 모델

기사 제목+요약을 벡터로 바꾼 뒤(문자 n-gram TF-IDF, 로컬 개발 환경에서는
다국어 문장 임베딩도 선택 가능) **로지스틱 회귀**로 분류합니다. 두 방식을
매번 같은 조건으로 교차검증해 더 나은 쪽을 씁니다.

실제 "학습"은 이 로지스틱 회귀가 라벨(👍=1, 👎=0)과 예측이 맞도록 가중치를
조정하는 과정입니다. 한 번도 라벨된 적 없는 새 기사에도 그대로 적용되어
관련도를 예측합니다 — 라벨 없이는 동작할 수 없는 지도학습이며, 상단
"키워드로 검색" 창이 쓰는 단순 유사도 비교(라벨·가중치 없이 매번 새로
계산)와는 다른 메커니즘입니다.

### 언제 다시 학습하나

라벨은 매주 지워지지 않고 계속 누적됩니다.

- 새 라벨이 **5건** 쌓일 때마다 재학습을 시도합니다.
- 그 시점까지 쌓인 라벨이 **30건**을 넘어야 실제로 모델을 씁니다(그
  이하면 "모델 없음"으로 취급해 최신순 정렬을 유지합니다).
- 재학습이 성공하면 **이번 주에 이미 수집된 기사도 그 자리에서 다시
  채점됩니다** — 다음 주까지 기다리지 않습니다.
- 다음 주 새로 수집되는 기사는 그 시점까지 누적된 최신 모델로 초벌
  점수가 매겨집니다.

좋아요 카운트 자체는 이와 별개로 항상 실시간 반영됩니다 — 👍를 누르면
그 즉시 화면 정렬(국내 매체 우선 → 좋아요 많은 순 → 분류기 점수순)에
반영되고, 이는 학습과 무관한 단순 카운트 비교입니다.

### 성능 측정

라벨을 그룹으로 나눠 교차검증합니다(라벨이 2주 이상 쌓이면 주차 단위,
아니면 같은 기사가 학습·검증에 겹치지 않도록 URL 단위) — 학습에 쓰인 적
없는 라벨로만 예측해 채점하므로 점수가 부풀려지지 않습니다.

| 지표 | 의미 |
| --- | --- |
| Precision | "관련 있다"고 예측한 것 중 실제로 관련 있던 비율 |
| Recall | 실제 관련 있는 것 중 모델이 찾아낸 비율 |
| F1 | Precision·Recall의 조화평균(두 방식 중 승자를 이걸로 가름) |
| Accuracy | 확률 0.5 기준으로 자른 예측이 맞은 비율 |
| AUC | 문턱 없이, 관련 기사에 항상 더 높은 확률을 줬는가 |
| Precision@10 / NDCG@10 | 팀별 상위 10건 안에 실제 관련 기사가 얼마나 몰렸는지 |

정확도는 항상 **"무조건 다수 쪽으로 찍었을 때"의 기준선**과 함께
표시됩니다 — 분류기가 이 기준선을 못 넘으면 모델을 아예 쓰지 않고 최신순
정렬로 돌아갑니다. 좋아요는 익명입니다 — 클릭마다 무작위 토큰이 발급되어
개수만 쌓이고 누가 눌렀는지는 저장되지 않습니다.
