...
**추가(2026-08-11) — Gartner 뉴스룸 + 기업 기술블로그 7종.** 담당자가 추가
조사한 목록을 RSS 존재 + robots.txt 준수 여부까지 직접 검증해 통과분만 추가.

- **Gartner 뉴스룸**(`/en/newsroom/rss`, 신뢰도 0.8): 011에서 "403 차단"으로
  적었던 건 다른 경로를 시도했다가 막힌 기록으로 보이고, 이 경로는 재검증 결과
  정상. 단 보도자료·시장 전망 피드일 뿐 매직 쿼드런트 등 애널리스트 리포트
  원문(구독 벽)은 여전히 못 가져온다.
- **기업 기술블로그**(우아한형제들·카카오·토스·당근·NHN Cloud·Meta 엔지니어링,
  신뢰도 0.65): 언론사가 아니라 회사가 직접 쓰는 1차 소스라 사실관계 오류
  위험은 낮지만 "자사 사례" 중심이라 시장 대표성은 낮게 잡음.
- **구글 코리아 개발자 블로그**(FeedBurner 경로, 신뢰도 0.7): 매주 Gemini/AI
  업데이트를 요약하는 다이제스트 성격이라 위 기업 블로그보다 소폭 상향.

확인했으나 보류: 아이티월드(robots.txt가 `/feed/` 전역 차단) · TechNeedle
(피드는 살아있으나 robots.txt가 `Disallow: /feed` 명시) · 티타임즈(RSS 없음,
robots.txt도 지정 크롤러만 허용) · "Dighty Data Market"(사이트 특정 실패,
URL 확인 후 재검토).

**실제 파이프라인 1회 실행 결과(2026-08-11 11:45 KST)**: 8개 중 6개(카카오·토스·
당근·NHN Cloud·Meta·구글 코리아)는 정상 수집되어 신규 기사 23건이 들어갔다.
Gartner 뉴스룸·우아한형제들 기술블로그 2개는 이 실행에서 HTTP 403을 반환했는데,
직후 curl로 재확인하니 둘 다 200으로 돌아와 있었다 — 위 GeekNews/전자신문과
같은 패턴의 일시적 봇 차단으로 보인다. 소스는 비활성화하지 않고 유지, 다음
정기 실행에서 재수집되는지 계속 관찰.
